<!-- Inspired by: https://github.com/E01T/simple-svelte-slideshow -->
<script>
	import { tweened } from 'svelte/motion';
	import { sineInOut, cubicInOut, expoInOut, backInOut, elasticInOut } from 'svelte/easing';
	...
	const getThumbnails = (fn) => {
		const thubmnails = Array.from(document.getElementsByClassName('thumbnails-img'));
	...
	const styleThumbnail = (array) => {
		...
		array[getCurrentIndex()].style.cssText = 'scale: 1.1; opacity: 1; z-index: 12';
	};
	...
	const selectImage = (e) => {
		...
		i =
			...
			(isImgDataset(e.target.dataset.img) && datasetIndex(+e.target.dataset.img));
...
{#if imgArrayLength}
	<div id="gallery">
		<figure>
			...
			/>
			<a on:click={selectImage} data-img={currentIndex - 1} id="previous">&#10094;</a>
			<a on:click={selectImage} data-img={currentIndex + 1} id="next">&#10095;</a>
			<figcaption>{currentImage.text || ''}</figcaption>
		</figure>
		<div on:click={selectImage} id="thumbnails">
			{#each imgArray as image, i}
				<img class="thumbnails-img" src={image.src} alt={image.text} data-img={i} />
			{/each}
		</div>
	</div>
{/if}
